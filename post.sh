# json
curl http://127.0.0.1:9490/invocations -X POST -H "Content-Type: application/json" -d '{
    "dataframe_split": {
        "columns": ["text"],
...
# csv
curl http://127.0.0.1:9490/invocations -X POST -H "Content-Type: text/csv" --data-binary "@post_file.csv"
