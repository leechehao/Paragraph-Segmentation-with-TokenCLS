...
def main():
    ...
    with mlflow.start_run(run_name=args.run_name) as run:
        ...
        for epoch in epochs:
            ...
            val_loss, val_f1, val_cls_report, val_duration = utils.evaluate(model, val_dataloader, device, label_list)
            epochs.write(f"Validation Loss: {val_loss:.4f}", file=log_file)
            epochs.write(f"Validation F1 (Macro): {val_f1:.4f}", file=log_file)
        ...
        # test_loss, test_f1, test_cls_report, test_duration = utils.evaluate(model, test_dataloader, device)
        # epochs.write(f"Test Loss: {test_loss:.4f}", file=log_file)
        # epochs.write(f"Test F1 (Macro): {test_f1:.4f}", file=log_file)
        ...
        sample = pd.DataFrame({"text": ["report"]})
        results = pd.DataFrame([{"Others": "others", "Findings": "findings", "Impression": "impression"}])
        signature = infer_signature(sample, results)

        mlflow.pyfunc.log_model(
            "model",
            python_model=utils.TokenCLSForParagraphSegmentationAPI(),
            code_path=["utils.py"],
            artifacts=artifacts,
            signature=signature,
        )
